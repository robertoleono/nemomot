// ============================================================
// NEMOMOT CART SYSTEM
// PayPal Standard (webscr) — simple redirect checkout
// ============================================================

const PAYPAL_CONFIG = {
  email: 'tabosas2011@gmail.com',
...
function addToCart(productId) {
  ...
  if (!product) return;
  if (cart.find(i => i.id === productId)) { openCart(); return; }
...
function injectCartStyles() {
  ...
  style.textContent = `
    ...
    .cart-header {
      ...
      padding: 1.25rem 1.5rem; border-bottom: .5px solid var(--rule, #ddd8ce);
    }
    .cart-header-title { font-family: var(--serif, Georgia); font-size: 1.05rem; font-weight: 400; color: var(--ink, #0e0e0e); }
    .cart-close { background: none; border: none; cursor: pointer; font-size: 14px; color: var(--ink-muted, #6b6560); padding: 4px; line-height: 1; transition: color .2s; }
    .cart-close:hover { color: var(--ink, #0e0e0e); }
    .cart-body { flex: 1; overflow-y: auto; padding: 1.25rem 1.5rem; }
    .cart-empty { font-size: 13.5px; color: var(--ink-muted, #6b6560); font-style: italic; font-family: var(--serif, Georgia); padding: 1rem 0; }
    .cart-item { display: flex; justify-content: space-between; align-items: flex-start; padding: 1rem 0; border-bottom: .5px solid var(--rule, #ddd8ce); gap: 1rem; }
    .cart-item:first-child { border-top: .5px solid var(--rule, #ddd8ce); }
    .cart-item-title { font-family: var(--serif, Georgia); font-size: 14.5px; color: var(--ink, #0e0e0e); margin-bottom: 2px; }
    .cart-item-author { font-size: 11.5px; color: var(--ink-muted, #6b6560); letter-spacing: .02em; }
    .cart-item-right { display: flex; align-items: center; gap: .75rem; flex-shrink: 0; }
    .cart-item-price { font-size: 13.5px; color: var(--ink, #0e0e0e); font-variant-numeric: tabular-nums; }
    .cart-item-remove { background: none; border: none; cursor: pointer; font-size: 11px; color: var(--ink-faint, #999); padding: 2px; transition: color .2s; line-height: 1; }
    .cart-item-remove:hover { color: var(--ink, #0e0e0e); }
    .cart-footer { padding: 1.25rem 1.5rem; border-top: .5px solid var(--rule, #ddd8ce); }
    .cart-total-row { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 1rem; }
    .cart-total-label { font-size: 10.5px; letter-spacing: .14em; text-transform: uppercase; color: var(--ink-muted, #6b6560); }
    .cart-total-amount { font-family: var(--serif, Georgia); font-size: 1.15rem; color: var(--ink, #0e0e0e); }
    .cart-checkout-btn {
      display: flex; align-items: center; justify-content: center; gap: .6rem;
      width: 100%; padding: 14px 20px; background: var(--gold, #b8924a);
      color: #0c0b08; border: none; font-size: 11px; letter-spacing: .13em;
      text-transform: uppercase; font-family: var(--sans, system-ui);
      font-weight: 500; cursor: pointer; transition: background .2s, opacity .2s;
      margin-bottom: .65rem;
    }
